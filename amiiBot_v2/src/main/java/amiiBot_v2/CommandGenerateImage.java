...
public class CommandGenerateImage {
	...
	public CommandGenerateImage(DiscordApi api, AmiiboHuntAccess inAmiiboData) {
		amiiboData = inAmiiboData;
		...
		thisCommand = new SlashCommandUpdater(thisCommand.getId()).setDescription(commandDescription)
				...
				.setSlashCommandOptions(Arrays.asList(
						SlashCommandOption.create(SlashCommandOptionType.SUB_COMMAND, "figures",
								"Generates an image of the figures in your collection!"),
						SlashCommandOption.createWithOptions(SlashCommandOptionType.SUB_COMMAND, "cards",
								...
								Arrays.asList(SlashCommandOption.createWithChoices(SlashCommandOptionType.STRING, "set",
										...
										SlashCommandOption.createWithChoices(SlashCommandOptionType.STRING, "subset",
												...
												Arrays.asList(SlashCommandOptionChoice.create("Wishlist", "wishlistOnly"),
														SlashCommandOptionChoice.create("Unobtained", "unownedOnly"),
														SlashCommandOptionChoice.create("Duplicates", "duplicatesOnly")))))))
		...
		api.addSlashCommandCreateListener(event -> {
				...
				slashCommandInteraction.respondLater().thenAccept(interactionOriginalResponseUpdater -> {
					...
					if (data.keySet().contains("error")) {
						...
						embed.setImage(data.getString("val"))
								.setTitle(event.getInteraction().getUser().getName() + "'s Figure Collection:");
					}
			...
			if (slashCommandInteraction.getFullCommandName().equals(commandName + " cards")) {
				slashCommandInteraction.respondLater().thenAccept(interactionOriginalResponseUpdater -> {
					EmbedBuilder embed = new EmbedBuilder();
					String series = slashCommandInteraction.getArguments().get(0).getStringValue().get();
					String set = null;
					if (slashCommandInteraction.getArguments().size() > 1) {
						set = slashCommandInteraction.getArguments().get(1).getStringValue().get();
					}
					JSONObject data = amiiboData.getCardImage(event.getInteraction().getUser().getIdAsString(), series, set);
					if (data.keySet().contains("error")) {
						embed.setColor(Color.red);
						switch (data.get("error").toString()) {
						...
						embed.setImage(data.getString("val"))
								.setTitle(event.getInteraction().getUser().getName() + "'s Card Collection:");
					}
					interactionOriginalResponseUpdater.addEmbed(embed).update().exceptionally(ExceptionLogger.get());
				});
			}
	...
	public void update(AmiiboHuntAccess newAmiiboData) {
